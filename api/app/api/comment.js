...
api.add = async (req, res) => {
    ...
    if(canComment) {
        ...
        console.log(`comment.js - Comentário adicionado`, comment);
        res.json(comment);
    } else {
        res.status(403).json({ message: 'comment.js - Proibido!'});
    }
};
...
api.listAllFromPhoto = async (req, res) => {
    ...
    const { photoId } = req.params;
    console.log(`comment.js - Receber comentários da foto ${photoId}`);
