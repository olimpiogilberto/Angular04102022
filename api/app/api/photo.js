...
api.list = async (req, res) => {
    ...
    if(user) {
        console.log(`photo.js - Listando fotos`);
        const photos = await new PhotoDao(req.db)
            .listAllFromUser(userName, page);
        res.json(photos);
    } else {
        res.status(404).json({ message: 'photo.js - Usuário não encontrado!'});
    }
...
api.add = async (req, res) => {
    console.log('#-----------photo.js--------------#');
    console.log('photo.js - Recebido dados JSON', req.body);
...
api.addUpload = async (req, res) => {
        ...
        console.log('Dados da foto', req.body);
...
api.findById = async (req, res) => {
    ...
    console.log('#----------------photo.js------------------------#');
    console.log(`photo.js - Procurando foto pelo ID ${photoId}`)
    const photo = await new PhotoDao(req.db).findById(photoId);
    if(photo) {
        res.json(photo);
    } else {
        res.status(404).json({ message: 'photo.js - Foto não existe!'})
    }  
};
...
api.remove = async (req, res) => {
    ...
    if(!photo) {
        const message = 'photo.js - Foto não existe!';
        console.log(message);
    ...
    if(userCanDelete(user)(photo)) {
        ...
        `);
        res.status(403).json({ message: 'photo.js - Proibido!'});
    }
};
